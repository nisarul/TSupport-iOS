...
final class PeerInfoScreenNode: ViewControllerTracingNode, UIScrollViewDelegate {
    ...
    private func editingOpenNotificationSettings() {
        ...
        let _ = (combineLatest(self.context.account.postbox.transaction { transaction -> (TelegramPeerNotificationSettings, GlobalNotificationSettings) in
            ...
            let globalSettings: GlobalNotificationSettings = transaction.getPreferencesEntry(key: PreferencesKeys.globalNotifications)?.get(GlobalNotificationSettings.self) ?? (self.context.account.isSupportAccount ? GlobalNotificationSettings.defaultSupportSettings : GlobalNotificationSettings.defaultSettings)
    ...
    fileprivate func updateNavigation(transition: ContainedViewLayoutTransition, additive: Bool) {
        ...
        if let (layout, navigationHeight) = self.validLayout {
            ...
            self.headerNode.navigationButtonContainer.update(size: CGSize(width: layout.size.width - layout.safeInsets.left * 2.0, height: navigationBarHeight), presentationData: self.presentationData, leftButtons: leftNavigationButtons, rightButtons: rightNavigationButtons, expandFraction: effectiveAreaExpansionFraction, transition: transition, isSupportAccount: self.context.account.isSupportAccount)
        }
    }
    ...
    public init(context: AccountContext, updatedPresentationData: (initial: PresentationData, signal: Signal<PresentationData, NoError>)?, peerId: PeerId, avatarInitiallyExpanded: Bool, isOpenedFromChat: Bool, nearbyPeerDistance: Int32?, callMessages: [Message], isSettings: Bool = false, hintGroupInCommon: PeerId? = nil, requestsContext: PeerInvitationImportersContext? = nil) {
        ...
        if isSettings {
            ...
            |> map { sharedData -> Bool in
                let settings = sharedData.entries[ApplicationSpecificSharedDataKeys.inAppNotificationSettings]?.get(InAppNotificationSettings.self) ?? (context.account.isSupportAccount ? InAppNotificationSettings.defaultSupportSettings : InAppNotificationSettings.defaultSettings)
