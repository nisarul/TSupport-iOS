...
@objc(AppDelegate) class AppDelegate: UIResponder, UIApplicationDelegate, PKPushRegistryDelegate, UNUserNotificationCenterDelegate, URLSessionDelegate, URLSessionTaskDelegate {
    ...
    private let supportAccountDbExpiryTimeDifference: Int = 2 * 60 * 60 // 2 hours
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]? = nil) -> Bool {
        ...
        let _ = self.deleteAccountDataIfRequired()
    ...
    func applicationDidEnterBackground(_ application: UIApplication) {
        ...
        // Save current time in seconds to file
        let currentTime = Int(Date().timeIntervalSince1970)
        let currentTimeString = String(currentTime)
        let fileManager = FileManager.default
        let documentDirectory = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first!
        let fileURL = documentDirectory.appendingPathComponent("lastTime.txt")
        do {
            try currentTimeString.write(to: fileURL, atomically: false, encoding: .utf8)
            print("SYD: Wrote current time \(currentTimeString) to file \(fileURL)")
            Logger.shared.log("App \(self.episodeId)", "SYD: Wrote current time \(currentTimeString) to file \(fileURL)")
        } catch {
            print("SYD: Error writing to file \(error)")
        }
    }
    ...
    func applicationWillEnterForeground(_ application: UIApplication) {
        ...
        let isAccountDbDeleted = self.deleteAccountDataIfRequired()
        if (isAccountDbDeleted) {
            print("SYD: Deleted account data, so exiting app")
            Logger.shared.log("App \(self.episodeId)", "SYD: Deleted account data, so exiting app")
            exit(0)
        }
    ...
    func deleteAccountDataIfRequired(force: Bool = false) -> Bool {
        // Create boolean variable to check if we should delete the account cache
        var shouldDeleteAccountCache = false

        if (!force) {
            // Get file contents from lastTime.txt
            let fileManager = FileManager.default
            let documentDirectory = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first!
            let fileURL = documentDirectory.appendingPathComponent("lastTime.txt")
            do {
                let fileContents = try String(contentsOf: fileURL, encoding: .utf8)
                Logger.shared.log("App \(self.episodeId)", "SYD: Read file contents \(fileContents) from file \(fileURL)")
                print("SYD: Read file contents \(fileContents) from file \(fileURL)")
                
                let lastTime = Int(fileContents)
                let currentTime = Int(Date().timeIntervalSince1970)
                let timeDifference = currentTime - lastTime!
                print("SYD: Time difference is \(timeDifference)")
                Logger.shared.log("App \(self.episodeId)", "SYD: Time difference is \(timeDifference)")
                let expiryTimeDifference = self.supportAccountDbExpiryTimeDifference
                if timeDifference > expiryTimeDifference {
                    print("SYD: Time difference is greater than \(expiryTimeDifference) seconds, so we should clear the account data in filemanager")
                    Logger.shared.log("App \(self.episodeId)", "SYD: Time difference is greater than \(expiryTimeDifference) seconds, so we should clear the account data in filemanager")
                    shouldDeleteAccountCache = true
                } else {
                    print("SYD: Time difference is \(timeDifference) seconds")
                    Logger.shared.log("App \(self.episodeId)", "SYD: Time difference is \(timeDifference) seconds")
                }
            } catch {
                print("SYD: Error reading from file \(error)")
                Logger.shared.log("App \(self.episodeId)", "SYD: Error reading from file \(error)")
            }
        } else {
            print("SYD: Force deleting account cache")
            Logger.shared.log("App \(self.episodeId)", "SYD: Force deleting account cache")
            shouldDeleteAccountCache = true
        }
            
        if (shouldDeleteAccountCache) {
            let baseAppBundleId = Bundle.main.bundleIdentifier!
            let appGroupName = "group.\(baseAppBundleId)"
            let maybeAppGroupUrl = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: appGroupName)
            guard let appGroupUrl = maybeAppGroupUrl else {
            ...
            if let contents = try? FileManager.default.contentsOfDirectory(at: URL(fileURLWithPath: rootPath), includingPropertiesForKeys: nil, options: [.skipsSubdirectoryDescendants]) {
                for url in contents {
                    print("App \(self.episodeId)", "root: \(url.path)")
                    if url.lastPathComponent.hasPrefix("account-") {
                        Logger.shared.log("App \(self.episodeId)", "SYD: root: \(url.path)")
                        if FileManager.default.fileExists(atPath: url.path + "/support_account") {
                            let databasePath = url.path + "/postbox/db"
                            let _ = try? FileManager.default.removeItem(atPath: databasePath)
                            Logger.shared.log("App \(self.episodeId)", "SYD: Deleted account \(url.lastPathComponent) \(URL(fileURLWithPath: url.path + "/postbox/db"))")
                            return true
                        } else {
                            Logger.shared.log("App \(self.episodeId)", "SYD: Not deleting account \(url.lastPathComponent) because it is not a support account")
                        }
                    }
                }
            }
    ...
    func applicationWillTerminate(_ application: UIApplication) {
        // print("SYD: applicationWillTerminate")
        ...
        do {
            ...
            let currentTime = Int(Date().timeIntervalSince1970)
            let timeDifference = currentTime - lastTime
            if timeDifference < self.supportAccountDbExpiryTimeDifference {
                print("SYD: Time difference is less than \(self.supportAccountDbExpiryTimeDifference) seconds, not writing to file")
                Logger.shared.log("App \(self.episodeId)", "SYD: Time difference is less than \(self.supportAccountDbExpiryTimeDifference) seconds, not writing to file")
            } else {
                let currentTimeString = String(currentTime)
                do {
                    try currentTimeString.write(to: fileURL, atomically: false, encoding: .utf8)
                    ...
                    print("SYD: Error writing to file \(error)")
                    Logger.shared.log("App \(self.episodeId)", "SYD: Error writing to file \(error)")
                }
            }
        } catch {
            print("SYD: Error reading from file \(error)")
    ...
    private func registerForNotifications(context: AccountContextImpl, authorize: Bool = true, completion: @escaping (Bool) -> Void = { _ in }) {
        ...
        let _ = (context.sharedContext.accountManager.transaction { transaction -> Bool in
            let settings = transaction.getSharedData(ApplicationSpecificSharedDataKeys.inAppNotificationSettings)?.get(InAppNotificationSettings.self) ?? (context.account.isSupportAccount ? InAppNotificationSettings.defaultSupportSettings : InAppNotificationSettings.defaultSettings)
