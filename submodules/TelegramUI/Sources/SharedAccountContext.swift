    ...
    private func performAccountSettingsImportIfNecessary() {
        ...
        if let primary = self.activeAccountsValue?.primary {
            ...
            |> deliverOnMainQueue).start(next: { [weak self] peer in
                ...
                if user.isPremium {
                    let _ = updateMediaDownloadSettingsInteractively(accountManager: self.accountManager, isSupportAccount: self.activeAccountsValue?.primary?.account.isSupportAccount ?? false, { settings in
                        var settings = settings
