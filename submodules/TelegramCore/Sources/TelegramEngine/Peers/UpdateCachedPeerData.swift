...
func _internal_fetchAndUpdateSupportCachedPeerData(accountPeerId: PeerId, peerId rawPeerId: PeerId, network: Network, postbox: Postbox) -> Signal<Bool, NoError> {
    return postbox.combinedView(keys: [.basicPeer(rawPeerId)])
                        ...
                        transaction.updatePeerCachedData(peerIds: [peerId], update: { peerId, current in
                            ...
                            switch result {
                                case .userInfo(let message, let entities, let author, let date):
                                    print("SYD: User Info: \(message), \(entities), \(author), \(date) - \(result)")
                                    let supportInfo = SupportInfo(message: message, author: author, date: Date.init(timeIntervalSince1970: TimeInterval(date)))
