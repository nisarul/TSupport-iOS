...
final class AccountTaskManager {
    private final class Impl {
        ...
        init(queue: Queue, stateManager: AccountStateManager, accountManager: AccountManager<TelegramAccountManagerTypes>,
             networkArguments: NetworkInitializationArguments, viewTracker: AccountViewTracker, mediaReferenceRevalidationContext: MediaReferenceRevalidationContext, isMainApp: Bool, testingEnvironment: Bool, isSupportAccount: Bool) {
            self.queue = queue
        ...
        private func stateManagerUpdated(isUpdating: Bool) {
            ...
            if isUpdating {
                ...
                if self.isMainApp {
                    ...
                    tasks.add(managedGlobalNotificationSettings(postbox: self.stateManager.postbox, network: self.stateManager.network, isSupportAccount: self.isSupportAccount).start())
    ...
    init(stateManager: AccountStateManager, accountManager: AccountManager<TelegramAccountManagerTypes>,
        ...
        self.impl = QueueLocalObject(queue: queue, generate: {
            return Impl(queue: queue, stateManager: stateManager, accountManager: accountManager, networkArguments: networkArguments, viewTracker: viewTracker, mediaReferenceRevalidationContext: mediaReferenceRevalidationContext, isMainApp: isMainApp, testingEnvironment: testingEnvironment, isSupportAccount: isSupportAccount)
        })
    }
}
