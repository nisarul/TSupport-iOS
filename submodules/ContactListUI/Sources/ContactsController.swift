...
public class ContactsController: ViewController {
    ...
    override public func loadDisplayNode() {
        ...
        self.contactsNode.openQrScan = { [weak self] in
            if let strongSelf = self {
                ...
                DeviceAccess.authorizeAccess(to: .camera(.qrCode), isSupportAccount: context.account.isSupportAccount, presentationData: presentationData, present: { c, a in
    ...
    @objc func addPressed() {
        let _ = (DeviceAccess.authorizationStatus(subject: .contacts, isSupportAccount: self.context.account.isSupportAccount)
        |> take(1)
        |> deliverOnMainQueue).start(next: { [weak self] status in
            guard let strongSelf = self else {
                return
